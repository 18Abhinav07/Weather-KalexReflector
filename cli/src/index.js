...
class WeatherFarmingCLI {
  ...
  async checkServerConnection() {
    const spinner = ora('Connecting to backend server...').start();
  ...
  async registerUser() {
    const { username, email } = await inquirer.prompt([
      {
        ...
        validate: input => input.length >= 3 ? true : 'Username must be at least 3 characters'
      },
      {
        type: 'input',
        name: 'email',
    ...
    const spinner = ora('Creating user account with custodial wallet...').start();
    ...
    const { userId } = await inquirer.prompt([
      {
        ...
        validate: input => input.length > 0 ? true : 'User ID is required'
      }
    ]);
  ...
  async createDemoUser() {
    const demoUsername = `demo_${Math.random().toString(36).substring(7)}`;
    const demoEmail = `${demoUsername}@demo.local`;

    const spinner = ora('Creating demo account...').start();
  ...
  async fetchCurrentCycle() {
    try {
      const response = await axios.get(`${this.baseURL}/cycles/current`);
      if (response.data.success) {
  ...
  async displayCycleStatus() {
    ...
    const cycle = this.currentCycle.cycle;
    const blockInCycle = parseInt(cycle.currentBlock) - parseInt(cycle.startBlock);
    const phaseInfo = this.getPhaseInfo(cycle.phase, blockInCycle);
  ...
  getPhaseInfo(phase, blockInCycle) {
    switch (phase) {
      case 'planting':
        return {
          icon: '🌱',
      ...
      case 'working':
        return {
          ...
          description: 'Only agriculture actions allowed. Weather wagers are closed. Real-world location reveal.'
        };
      case 'revealing':
        return {
          icon: '🎭',
  ...
  async handleStayAction() {
    await this.submitAction('stay', {});
  }

  async submitAction(actionType, actionData) {
    const spinner = ora(`Submitting ${actionType} action...`).start();

    try {
      const response = await axios.post(`${this.baseURL}/cycles/action`, {
        userId: this.userId,
        actionType,
        actionData
      });
  ...
  async viewUserPositions() {
    const spinner = ora('Fetching your farming positions...').start();

    try {
      const response = await axios.get(`${this.baseURL}/users/${this.userId}/positions`);
  ...
  async viewTransactionHistory() {
    ...
    try {
      const response = await axios.get(`${this.baseURL}/users/${this.userId}/transactions`);
  ...
  async viewDAOStatistics() {
    const spinner = ora('Fetching DAO performance statistics...').start();

    try {
      const response = await axios.get(`${this.baseURL}/weather/statistics`);
  ...
  async startRealTimeMonitoring() {
    ...
    const monitor = async () => {
      try {
        const response = await axios.get(`${this.baseURL}/cycles/live-feed`);
          ...
          if (feed.cycle && feed.cycle.currentBlock !== lastBlock) {
            ...
            if (feed.prompt) {
              ...
              if (feed.prompt.actions && feed.prompt.actions.length > 0) {
                console.log(chalk.gray(`Available actions: ${feed.prompt.actions.map(a => a.label).join(', ')}`));
              }
            }
          }
        }
